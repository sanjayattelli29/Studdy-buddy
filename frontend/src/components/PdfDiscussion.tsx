...
const PdfViewer: React.FC<PdfViewerProps> = ({ pdfUrl, fileName, onClose }) => {
  ...
  const [viewerMethod, setViewerMethod] = useState<'iframe' | 'object' | 'download'>('iframe');
        ...
        <div className="pdf-viewer-content">
          ...
          {error && (
            <div className="pdf-error">
              ...
              <p>The PDF file might be corrupted or incompatible with the browser viewer.</p>
          ...
          {viewerMethod === 'iframe' ? (
            ...
            <object
              ...
              onError={handleError}
            >
              <p>Your browser doesn't support PDFs. <button onClick={downloadPdf}>Download the PDF</button>.</p>
            </object>
          )}
        </div>
      </div>
...
const PdfDiscussion: React.FC<PdfDiscussionProps> = ({ roomId }) => {
  ...
  const deleteFolder = async (folderId: string) => {
    if (!window.confirm('Are you sure you want to delete this folder and all its files?')) return;
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        // Upload to R2 with folder structure
        const folderPath = `study-rooms/${roomId}/pdfs/${selectedFolder}`;
        const uploadResult = await uploadToR2(file, folderPath);
        ...
        await addDoc(collection(db, 'pdfFiles'), {
          ...
          uploadedByName: currentUser.displayName || currentUser.email || 'Anonymous',
        ...
        <div className="folders-sidebar">
          ...
          <div className="folders-list">
            {folders.length === 0 ? (
              ...
              folders.map(folder => {
                const folderFiles = pdfFiles.filter(file => file.folderId === folder.id);
