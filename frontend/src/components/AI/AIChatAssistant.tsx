...
const AIChatAssistant: React.FC<AIChatAssistantProps> = ({ roomId }) => {
  ...
  const [showReactionPicker, setShowReactionPicker] = useState<string | null>(null); // Store message ID for which reaction picker is shown
  ...
  const processAIQuery = async (query: string): Promise<string> => {
    try {
      ...
      // Send the question and answer to the n8n webhook in the background
      ...
      .then(() => console.log("Question and answer sent to n8n webhook"))
      .catch(webhookError => console.error('Error sending to webhook:', webhookError));
  ...
  const processFileAnalysis = async (file: File): Promise<string> => {
    try {
      ...
      if (fileType.includes('pdf') || file.name.toLowerCase().endsWith('.pdf')) {
        ...
        console.error('Unsupported file type:', fileType, 'File name:', file.name);
...
4. If academic, explain core concepts simply`;
      ...
      .then(() => console.log("File analysis request and response sent to n8n webhook"))
      .catch(webhookError => console.error('Error sending file analysis to webhook:', webhookError));
      ...
      if (error.message.includes('No text content found')) {
        throw new Error('This file appears to be empty or contains no readable text. Please check if the file is valid.');
      } else if (error.message.includes('Unsupported file type')) {
        throw new Error('Only PDF and DOC/DOCX files are supported. Please upload a different file type.');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const queryText = isAIQuery ? messageText.substring(3).trim() : messageText;
      ...
      if (file) {
        ...
        try {
          ...
          if (err instanceof Error) {
            if (err.message.includes('No text content found') || err.message.includes('empty')) {
              errorMessage = "This file appears to be empty or contains no readable text. Please check if it's a valid document.";
            } else if (err.message.includes('Unsupported file type')) {
              errorMessage = "Only PDF and DOC/DOCX files are supported. Please upload a different file type.";
            } else if (err.message.includes('API error') || err.message.includes('processing your request')) {
              errorMessage = "There was an issue processing your request. Please try again in a moment.";
            } else {
          ...
          setError('Document analysis failed. Please try uploading a different file.');
        } finally {
      ...
      // Handle AI query
        ...
        try {
          ...
          setMessages(prev => prev.map(msg => 
            msg.id === aiMessageId 
              ? { ...msg, content: "Sorry, I couldn't process your request. Please try again later.", isProcessing: false } 
              : msg
          ));
  ...
  const toggleReactionPicker = (messageId: string) => {
    setShowReactionPicker(current => current === messageId ? null : messageId);
  };
  ...
  const renderMessage = (message: AIChatMessage) => {
    ...
    const messageClass = isCurrentUser ? 'user-message' : message.isAI ? 'ai-message' : 'other-message';
        ...
        {messages.length === 0 ? (
          <div className="no-messages">
            <div className="ai-welcome-message">
              ...
              <p>Ask me anything by starting your message with <strong>@ai</strong></p>
              <p>Examples:</p>
              <ul>
