...
const CodingIDE: React.FC<CodingIDEProps> = ({ roomId }) => {
  ...
  const [chatMessages, setChatMessages] = useState([
    {
      role: 'assistant',
      content: 'Hi! I can help you with coding. Ask me anything or request code examples!'
    }
  ]);
  ...
  const sendQuery = async (e) => {
    ...
    try {
      ...
      const assistantMessage = { role: 'assistant', content: result.choices[0].message.content };
      ...
      const codeBlockMatch = assistantMessage.content.match(/```python\n([\s\S]*?)```/);
      if (codeBlockMatch && codeBlockMatch[1]) {
        ...
        setTimeout(() => {
          ...
          const lastMsg = document.querySelector('.chatbot-messages .message:last-child .message-content');
          if (lastMsg) {
          ...
          <div className="chatbot-container">
            ...
            <form className="chatbot-input" onSubmit={sendQuery}>
              <input
                ...
                placeholder={isProcessingQuery ? "Thinking..." : "Ask coding questions or request examples..."}
              ...
              >
                {isProcessingQuery ? <RefreshCw size={16} className="spin" /> : 'Send'}
              </button>
            </form>
          </div>
        )}
